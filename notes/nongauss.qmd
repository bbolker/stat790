---
title: "Non-Gaussian responses (week 2)"
...
- still unbiased but not minimum variance
...
$$
\log \left( \frac{\textrm{Pr}(G=i|X=x)}{\textrm{Pr}(G=K|X=x)} = \beta_{i0} + \beta_i^\top x, \quad $i \in 1 \ldots K-1$
$$
(and so $\textrm{Pr}(G=K|X=x) = 1/\left(1 + \sum_{i=1}^{K-1} \exp(\beta_{i0} + \eta_i^\top x)\right)$)

* independent of baseline/reparameterization
* log-likelihood $\sum \log p__i(x_i; \theta)$ where $\theta$ is the complete set of parameters
...
https://bbolker.github.io/stat4c03/notes/glm_deriv.pdf
