...
pfun <- function(k) {
    plot(rotated(k),col= cc[penguins$species], pch = 16,
         xlab = "PC1", ylab = "PC2")
}
...
```{r}
pfun2 <- function(p) {
    plot(p$score[,1], p$score[,2], col= cc[penguins$species], pch = 16,
         xlab = "PC1", ylab = "PC2")
}
## not working ...
expand_X <- function(X, sigma = 1, n = 10) {
    res <- list()
    for (m in 1:n) {
        res[[m]] <- apply(X/sigma, 2, function(x) x^m/factorial(m))
    }
    do.call("cbind", res)
}
eX <- expand_X(pX)
par(mfrow= c(1,2))
pfun(k0); pfun2(princomp(pX))
## not identical but close?
pfun(k1); pfun2(princomp(~.^2, data = as.data.frame(pX)))
## pfun(k3); pfun2(princomp(eX))
```
