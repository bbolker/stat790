...
$$
R = \frac{1}{2} \sum_{i=1}^N \left( y_i - \sum_{\k neq j} x_ik \bkl - x_{ij} \beta_j \right)^2 + \lambda \sum_{k \neq j} |\bkl| + \lambda |\beta_j|
$$
...
$$
\bjl \from S\left( \sum_{i=1}^N x_{ij}(y_i - \tilde y_i^{(j)}), \lambda \right)
$$
...
* ESL \S 3.8.4
* lasso on groups of parameters: compute $||
