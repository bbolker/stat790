...
* steepest descent; find gradient, line search
...
* fit a decision tree (learner) to pseudo-residuals
...
* for MSE $\gamma_m = 1/N \sum (h_m(x) (\hat y - y))/\sum (h_m^2)$ 
...
* $\gamma_{jm}$ reduces to the mean $(\hat y-y)$
...
* use a "Newton" step (**elementwise** second-order approximation)
   ...
   * irrelevant for MSE, Huber loss, L1 loss ...
